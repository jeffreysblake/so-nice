...
2. **Detects monorepo structure** (if present):
   - Checks for npm/yarn/pnpm workspaces
   - Automatically runs `/setup-monorepo` if detected
   - Generates `.claude/memory/workspace-map.md`

3. **Scans project files** for technology fingerprints:
...
4. **Parses dependencies** to identify frameworks and tools:
...
5. **Matches conditional templates** from `.claude/conditionals/tech-stacks/`:
...
6. **Extracts content from best practice files**:
...
8. **Generates `.claude/memory/loaded_templates.md`**:
