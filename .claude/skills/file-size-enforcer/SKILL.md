...
description: Enforces strict 400-line file size limit. Use when files approach 350 lines, when splitting files, or when user mentions file size. Provides splitting strategies for all languages and file types. Alerts proactively before limit is reached. (project)
---
...
Provides advisory guidance to keep new/modified source files under 400 lines (test files excluded).

## When to Use

Claude automatically invokes this skill when:
- Creating new source files that approach 350 lines
- Modifying existing files and additions would push over 400 lines
- User mentions "file too large", "split file", or "file size"
- Adding significant code to a file

## The 400-Line Guideline

**ADVISORY GUIDELINE**: New/modified source files should stay under 400 lines.

**Exclusions:**
- Test files (*.test.*, *.spec.*, *_test.*, etc.) - no limit
- Legacy files already >400 lines - don't require immediate refactoring
- Generated files - follow upstream conventions
...
## Warning Thresholds (New/Modified Files Only)

- **350+ lines**: ⚠️ Consider splitting for maintainability
- **380+ lines**: 🟡 Suggest splitting before adding more
- **400+ lines**: 🟠 Recommend splitting into modules
...
The pre-commit hook warns about new/modified files exceeding 400 lines:

```bash
# .git/hooks/pre-commit
for file in $(git diff --cached --name-only --diff-filter=AM | grep -E '\.(py|ts|tsx|js|jsx|java|go|rs)$' | grep -v -E '\.(test|spec)\.'); do
    lines=$(wc -l < "$file")
    if [ "$lines" -gt 400 ]; then
        echo "⚠️  File $file exceeds 400 lines ($lines lines)"
        echo "   Consider splitting for better maintainability"
    fi
done
```

Note: This is advisory only. Legacy files >400 lines won't trigger warnings.
...
- **400 lines is an advisory guideline** for new/modified source files
- **Test files are excluded** from size limits (they can be large)
- **Legacy files >400 lines** don't need immediate refactoring
- **Split proactively** at 350 lines when creating new code
- **Split by domain/responsibility**, not arbitrarily
- **Test after splitting** to ensure nothing broke
- **Focus on new code quality**, respect existing codebase conventions
