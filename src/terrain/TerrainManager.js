...
export class TerrainManager {
     ...
     */
    buildFlatSection(startX, length, y) {
     ...
     */
    buildHill(startX, endX, baseY, height) {
     ...
     */
    buildRunwayToLoop(startX, length, y) {
        for (let x = startX; x < startX + length; x++) {
     ...
     */
    buildFullLoop(centerX, centerY, radius) {
        // Build circular loop using curve tiles
        const diameter = radius * 2;
        // Bottom approach
        for (let i = -2; i < 0; i++) {
            this.collisionManager.setTile(centerX + i, centerY + radius, TerrainTiles.createTile(TerrainTiles.FLAT));
        }
        ...
        for (let i = 0; i < diameter; i++) {
            this.collisionManager.setTile(centerX - radius + i, centerY - radius, TerrainTiles.createTile(TerrainTiles.FLAT));
        }
    ...
    buildPathSplit(startX, baseY) {
        ...
        // Ramp to high path
        for (let i = 0; i < 4; i++) {
            this.collisionManager.setTile(startX + i, baseY - i, TerrainTiles.createTile(TerrainTiles.SLOPE_45_UP));
        }
        // Ramp down from high path
        for (let i = 0; i < 4; i++) {
            this.collisionManager.setTile(startX + 26 + i, baseY - 8 + i, TerrainTiles.createTile(TerrainTiles.SLOPE_45_DOWN));
        }
    }
    /**
     * Build downhill speed run
     */
    buildDownhillRun(startX, baseY) {
        for (let i = 0; i < 15; i++) {
            const y = baseY - 10 + i;
            this.collisionManager.setTile(startX + i, y, TerrainTiles.createTile(TerrainTiles.SLOPE_45_DOWN));
        }
     ...
     */
    buildValley(startX, baseY) {
        // Down into valley
        for (let i = 0; i < 6; i++) {
            this.collisionManager.setTile(startX + i, baseY + i, TerrainTiles.createTile(TerrainTiles.SLOPE_45_DOWN));
        }
        // Valley floor
        this.buildFlatSection(startX + 6, 4, baseY + 6);
        // Up out of valley
        for (let i = 0; i < 6; i++) {
            this.collisionManager.setTile(startX + 10 + i, baseY + 6 - i, TerrainTiles.createTile(TerrainTiles.SLOPE_45_UP));
        }
    }
    /**
     * Build platforming section with gaps
     */
    buildPlatformSection(startX, baseY) {
        // Platform 1
