...
describe('CollisionManager', () => {
    ...
    describe('Sensor Casting - Flat Ground', () => {
        ...
        it('should detect collision when sensor is on flat surface', () => {
            ...
            const result = collisionManager.castSensor(8, 160, SensorDirection.DOWN, GroundMode.FLOOR);
            expect(result.collided).toBe(true);
            expect(result.angle).toBe(0);
        });
        it('should not detect collision when sensor is above surface', () => {
            // Sensor well above the ground
            const result = collisionManager.castSensor(8, 100, SensorDirection.DOWN, GroundMode.FLOOR);
            expect(result.collided).toBe(false);
        });
        it('should calculate correct distance to surface', () => {
            // Sensor at Y=154, surface at Y=160
            const result = collisionManager.castSensor(8, 154, SensorDirection.DOWN, GroundMode.FLOOR);
            expect(result.collided).toBe(false);
    ...
    describe('Sensor Casting - Slopes', () => {
        ...
        it('should detect collision on slope', () => {
            ...
            const result = collisionManager.castSensor(8, // Middle of tile
            168, // Near bottom
            SensorDirection.DOWN, GroundMode.FLOOR);
            expect(result.collided).toBe(true);
        ...
        it('should return correct angle for slope', () => {
            ...
            const result = collisionManager.castSensor(88, // Grid X=5, local X=8
    ...
    describe('Ground Sensors (Dual Sensor)', () => {
        ...
        it('should use both left and right sensors', () => {
            const result = collisionManager.checkGroundSensors(80, // Center X (grid 5)
            160, // Y position
        ...
        it('should detect ground when only one sensor hits', () => {
            ...
            const result = collisionManager.checkGroundSensors(94, // Near edge
            160, 9, GroundMode.FLOOR);
    ...
    describe('Adjacent Tile Detection', () => {
        it('should check adjacent tiles when needed', () => {
            ...
            const result = collisionManager.castSensor(8, 165, // In tile 10, but close to tile 11
            SensorDirection.DOWN, GroundMode.FLOOR);
    ...
    describe('Edge Cases', () => {
        it('should handle sensors at tile boundaries', () => {
            ...
            const result = collisionManager.castSensor(16, // Boundary between tiles
            160, SensorDirection.DOWN, GroundMode.FLOOR);
            expect(result.distance).toBeGreaterThanOrEqual(0);
        });
        it('should handle sensors in empty space', () => {
            // No tiles set
            const result = collisionManager.castSensor(1000, 1000, SensorDirection.DOWN, GroundMode.FLOOR);
            expect(result.collided).toBe(false);
            expect(result.distance).toBeGreaterThan(0);
        });
        it('should handle negative coordinates', () => {
            ...
            const result = collisionManager.castSensor(-8, -8, SensorDirection.DOWN, GroundMode.FLOOR);
